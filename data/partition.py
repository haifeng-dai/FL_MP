...
class TensorSubset[Sample](Dataset[Sample]):
    """以共享 CPU Tensor 保存索引的轻量数据集视图。"""

    def __init__(self, dataset: Dataset[Sample], indices: torch.Tensor):
        if indices.ndim != 1 or indices.device.type != "cpu":
            raise ValueError("索引必须是一维 CPU Tensor")
...
def assemble_subsets[Sample](
    dataset: Dataset[Sample],
    client_indices: dict[int, torch.Tensor],
) -> dict[int, TensorSubset[Sample]]:
    """将客户端索引组装为共享 Tensor 索引的数据集视图。"""
    return {
        client_id: TensorSubset(dataset, indices)
...
def dirichlet_partition_with_plan(
    ...
    for label in torch.unique(labels, sorted=True):
        ...
        for client_id, chunk in enumerate(
            torch.tensor_split(shuffled, boundaries.tolist())
        ):
            parts[client_id].append(chunk)
    return _merge_parts(parts)
...
def _assign_client_classes(
    ...
    for label in shuffled_classes.tolist():
        ...
        assignments[balanced[int(choice)]].add(label)
