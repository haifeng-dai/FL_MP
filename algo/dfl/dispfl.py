...
class DisPFLClient(BaseClient):
    def train(self, task):
        ...
        loss.backward()
        round_ratio = torch.tensor(
            task.payload["round"] / task.payload["num_rounds"], device=self.device
        )
        alpha = (
            task.payload["anneal_factor"]
            * 0.5
            * (1 + torch.cos(round_ratio * torch.pi))
        )
        updated = {}
        for name, parameter in self.model.named_parameters():
            ...
            remove = min(torch.ceil(alpha * active).to(torch.int64).item(), active)
            if remove:
                active_weights = torch.where(
                    mask.bool(), parameter.data.abs(), torch.inf
                )
                removed = (
                    active_weights.flatten().topk(int(remove), largest=False).indices
                )
                mask.flatten()[removed] = 0
                available = int((mask == 0).sum().item())
                regrow = int(min(remove, available))
